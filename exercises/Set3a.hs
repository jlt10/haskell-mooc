...
multiCompose (f : gs) value = f (multiCompose gs value)
...
interpreter' x y commands = case commands of
  [] -> []
  ("up":rest) -> interpreter' x (y + 1) rest
  ("down":rest) -> interpreter' x (y - 1) rest
  ("right":rest) -> interpreter' (x + 1) y rest
  ("left":rest) -> interpreter' (x - 1) y rest
  ("printX":rest) -> show x : interpreter' x y rest 
  ("printY":rest) -> show y : interpreter' x y rest 
  (_:rest) -> interpreter' x y rest   -- Unrecognized command
